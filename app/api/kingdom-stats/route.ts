...
const upsertSchema = z.object({
  ...
  iconKey: z.string().min(1).max(60).default("Trophy"),
  order: z.number().int().min(0).default(0),
  active: z.boolean().default(true),
});

// PUBLIC: list active kingdom stats
export async function GET(request: Request) {
  try {
    const items = await prisma.kingdomStat.findMany({
      where: { active: true },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const item = await prisma.kingdomStat.create({ data });
